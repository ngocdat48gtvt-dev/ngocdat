...
type SheetStyles = {
  ...
  subtitle: Partial<ExcelJS.Style>
...
function writeBaoLuHeader(sheet: ExcelJS.Worksheet, styles: SheetStyles, colCount: number) {
...
async function buildBaoLuWorkbook(items: IncidentRecord[]): Promise<ExcelJS.Workbook> {
  const lastCol = 10
...
async function buildStandardWorkbook(items: IncidentRecord[]): Promise<ExcelJS.Workbook> {
  const lastCol = 8
  const colCount = lastCol + 1
